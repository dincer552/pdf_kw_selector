...
def match_project_names(left: str | None, right: str | None, *, left_source: str | None = None, right_source: str | None = None) -> ProjectMatch:
    ...
    debug("Proje eşleşmesi hesaplandı", left=left, right=right, score=result.score, status=status, reason=reason)
    return result
...
def match_discoveries(left: ProjectDiscovery, right: ProjectDiscovery) -> ProjectMatch:
    ...
    try:
        ...
        if status in {"REVIEW_REQUIRED", "NO_MATCH"}:
            warning("Proje eşleşmesi kesin değil", left=left_raw, right=right_raw, score=result.score, status=status, reason=reason)
        return result
    except Exception as exc:
...
def match_discovery_lists(left_items: list[ProjectDiscovery], right_items: list[ProjectDiscovery]) -> list[ProjectMatch]:
    ...
    try:
        ...
        exception("Toplu proje eşleştirme hesaplama hatası", exc, pdf1_count=len(left_items), pdf2_count=len(right_items))
        raise
