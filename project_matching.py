...
def score_project_names(left: str | None, right: str | None) -> tuple[float, str, str]:
    try:
        ...
        if score >= 0.45:
            return score, "REVIEW_REQUIRED", "similar project names require user confirmation"
...
def match_project_names(left: str | None, right: str | None, *, left_source: str | None = None, right_source: str | None = None) -> ProjectMatch:
    ...
    return ProjectMatch(_matching_display_name(left), _matching_display_name(right), normalize_project_name(left or "") or None, normalize_project_name(right or "") or None, round(score, 4), status, reason, left_source, right_source)
...
def match_discoveries(left: ProjectDiscovery, right: ProjectDiscovery) -> ProjectMatch:
    try:
        ...
        result = ProjectMatch(
            _matching_display_name(left_raw),
            _matching_display_name(right_raw),
