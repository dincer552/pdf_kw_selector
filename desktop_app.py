...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(top, text="PDF kW SELECTOR", font=("Segoe UI", 18, "bold")).pack(side="left")
        ttk.Label(top, text=f"{VERSION} • Project → AHU → Motor").pack(side="right", pady=8)
        boxes = ttk.Frame(self, padding=(8, 0)); boxes.pack(fill="x")
        self.pdf1_label, self.pdf1_box = self._file_box(boxes, "PDF 1 — Seçim / Referans", "PDF1")
        self.pdf2_label, self.pdf2_box = self._file_box(boxes, "PDF 2 — Elektrik / Üretim", "PDF2")
        self.pdf1_box.pack(side="left", fill="x", expand=True, padx=(0, 5))
        self.pdf2_box.pack(side="left", fill="x", expand=True, padx=(5, 0))
        tabs = ttk.Notebook(self); tabs.pack(fill="both", expand=True, padx=8, pady=(6, 0))
        result_tab = ttk.Frame(tabs); log_tab = ttk.Frame(tabs)
        tabs.add(result_tab, text="SONUÇLAR"); tabs.add(log_tab, text="HATA / İŞLEM LOGLARI")
        cols = ("Proje","AHU","Motor","Tip","PDF1 kW","PDF2 kW","Fark","Durum","PDF1","PDF2")
        ...
        detail_frame = ttk.LabelFrame(self, text="Sonuç JSON / teknik detay", padding=5); detail_frame.pack(fill="both", expand=False, padx=8, pady=4)
        self.detail = tk.Text(detail_frame, height=6, wrap="none"); self.detail.pack(fill="both", expand=True)
        self.detail.configure(state="disabled")
        buttons = ttk.Frame(self, padding=5); buttons.pack(fill="x")
        ttk.Button(buttons, text="TOPLU ANALİZ", command=self.compare).pack(side="left", padx=3)
        ttk.Button(buttons, text="SEÇİMLERİ TEMİZLE", command=self.clear_inputs).pack(side="left", padx=3)
        ttk.Button(buttons, text="JSON KAYDET", command=self.save_json).pack(side="left", padx=3)
        ttk.Button(buttons, text="GÜNCELLEME KONTROL ET", command=self.check_updates).pack(side="left", padx=3)
        self.status = ttk.Label(buttons, text="Hazır", anchor="e"); self.status.pack(side="right")
        self.log_text = tk.Text(log_tab, wrap="none"); self.log_text.pack(fill="both", expand=True, padx=5, pady=5)
        log_buttons = ttk.Frame(log_tab, padding=5); log_buttons.pack(fill="x")
        ttk.Button(log_buttons, text="LOGLARI YENİLE", command=self.refresh_logs).pack(side="left", padx=3)
        ttk.Button(log_buttons, text="LOG DOSYASINI AÇ", command=self.open_log_file).pack(side="left", padx=3)
        ttk.Button(log_buttons, text="LOG KLASÖRÜNÜ AÇ", command=self.open_log_directory).pack(side="left", padx=3)
        ttk.Button(log_buttons, text="LOGLARI TEMİZLE", command=self.clear_logs).pack(side="left", padx=3)
        self.refresh_logs()

    def _file_box(self, parent, title, side):
        frame = ttk.LabelFrame(parent, text=title, padding=6)
        label = ttk.Label(frame, text="0 PDF seçildi")
        label.pack(side="left", fill="x", expand=True)
        ttk.Button(frame, text="PDF EKLE", command=lambda: self.add_files(side)).pack(side="right", padx=2)
        ttk.Button(frame, text="KLASÖR EKLE", command=lambda: self.add_folder(side)).pack(side="right", padx=2)
        return label, frame

    def add_files(self, side):
        paths = filedialog.askopenfilenames(title=f"{side} PDF seç", filetypes=[("PDF", "*.pdf")])
        self._merge_inputs(side, list(paths))
    ...
    def _merge_inputs(self, side, paths):
        ...
        label = self.pdf1_label if side == "PDF1" else self.pdf2_label
        names = ", ".join(Path(x.path).name for x in target[:3])
        label.configure(text=f"{len(target)} PDF: {names}{' ...' if len(target) > 3 else ''}")
