...
_PROJECT_RE = re.compile(r"^\s*project\b\s*[:=]?\s*(.*?)\s*$", re.I)
...
def discover_project_from_text(pages: list[str]) -> ProjectDiscovery:
    try:
        ...
        for page_number, text in enumerate(pages, 1):
            ...
            for line in expanded.splitlines(): fragments.append((page_number, line.strip()))
        lines = [line for _, line in fragments]
