...
def _find_multiline_project_name(lines: list[str], start_index: int) -> str:
    ...
    while look < limit:
        ...
        if _is_field_label(value) or _is_known_field_value(value):
            debug("Proje adı adayı teknik alan olarak atlandı", candidate=value, line_index=look)
            look += 1
            continue
        if _looks_like_project_name(value):
            return _strip_header_metadata(value)
        look += 1
    warning("Boş Project Name sonrasında geçerli proje adı bulunamadı", start_index=start_index)
    return ""
...
def discover_project_from_text(pages: list[str]) -> ProjectDiscovery:
    try:
        ...
        if selected:
            info("Proje adı keşfedildi", project=selected.value, normalized=selected.normalized, source=selected.source, page=selected.page, confidence=selected.confidence, candidate_count=len(unique))
        else:
            warning("Proje adı keşfedilemedi", candidate_count=len(unique), candidates=[x.to_dict() for x in unique])
        return result
    except Exception as exc:
        exception("Proje keşfi hesaplama hatası", exc, page_count=len(pages))
        raise
...
def discover_project(path: str | Path) -> ProjectDiscovery:
    try:
        ...
        exception("PDF proje keşfi başarısız", exc, path=str(path))
        raise
