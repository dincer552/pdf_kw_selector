...
class GroupedApp(BaseApp):
    ...
    def _grouped_scan_pdf1(self,document):
        ...
        if scan is None:
            scan=scan_pdf(document.path,"PDF1"); self._grouped_pdf1_scan_cache[key]=scan
        return scan
    def _special_pdf_sets(self):
        ebm=set(); voc=set(); sysr=set()
    ...
    def _render_sysreco(self):
        ...
        self.tabs.tab(self.sysreco_tab,text=f"SYSRECO ({len(self._sysreco_pdf_keys)})"); info("SysReco sonuçları oluşturuldu",row_count=len(rows),pdf_count=len(self._sysreco_pdf_keys))
    ...
    def _matched_pdf_keys(self):
        result=set(); accepted_statuses={"EXACT","NORMALIZED_MATCH","USER_APPROVED"}
        for ahu in self.analysis.ahu_matches:
    ...
    def _render_unmatched(self):
        ...
        self._special_pdf_sets(); matched_paths=self._matched_pdf_keys(); all_keys=self._selected_pdf_keys(); rows=[]; analyzed_keys=set()
        for side,documents in (("PDF1",self.analysis.pdf1_documents),("PDF2",self.analysis.pdf2_documents)):
            for document in documents:
                key=(side,str(document.path).casefold()); analyzed_keys.add(key)
                if key in matched_paths or key in self._ebm_pdf_keys or key in self._vocclean_pdf_keys or key in self._sysreco_pdf_keys:continue
        ...
        for key in sorted(all_keys-analyzed_keys):
            side,path=key; rows.append((side,Path(path).name,"-","-","PDF analiz dışında kaldı",path))
        ...
        self._unmatched_pdf_keys={(side,str(path).casefold()) for side,_,_,_,_,path in rows}; self.tabs.tab(self.unmatched_tab_index(),text=f"EŞLEŞMEYEN PDF'LER ({len(self._unmatched_pdf_keys)})")
        info("Eşleşmeyen PDF listesi oluşturuldu",unmatched_count=len(self._unmatched_pdf_keys),matched_ahu_pdf_count=len(matched_paths))
    def _clear_grouped_results(self):
        for item in self.ebm_tree.get_children():self.ebm_tree.delete(item)
        for item in self.voclean_tree.get_children():self.voclean_tree.delete(item)
        for item in self.sysreco_tree.get_children():self.sysreco_tree.delete(item)
        self._grouped_pdf1_scan_cache.clear(); self._ebm_pdf_keys.clear(); self._vocclean_pdf_keys.clear(); self._sysreco_pdf_keys.clear(); self._unmatched_pdf_keys.clear(); self.tabs.tab(0,text="DANFOS (0)"); self.tabs.tab(self.ebm_tab,text="EBM-PAPST (0)"); self.tabs.tab(self.voclean_tab,text="VOCLEAN (0)"); self.tabs.tab(self.sysreco_tab,text="SYSRECO (0)")
