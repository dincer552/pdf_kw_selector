...
def _dedupe(results: list[PDF2MotorResult]) -> list[PDF2MotorResult]:
    ...
    for result in results:
        key = (result.equipment_id.upper(), result.component_type, result.value_kw)
        if key in seen:
            continue
        seen.add(key)
...
def find_pdf2_motor_powers(path: str | Path) -> list[PDF2MotorResult]:
    ...
    for page_number, text in enumerate(pages, 1):
        ...
        if not result:
            continue
        if result.component_role in summary:
            _, quantity = summary[result.component_role]
            result = PDF2MotorResult(
                result.equipment_id,
