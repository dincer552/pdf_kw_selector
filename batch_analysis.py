...
class BatchAHU:
    ...
    def to_dict(self) -> dict:
        return {"project_name": self.project_name, "match": self.match.to_dict(), "pdf1_files": list(self.pdf1_files), "pdf2_files": list(self.pdf2_files)}
...
class BatchAnalysis:
    ...
    def to_dict(self) -> dict:
        return {"pdf1_documents": [x.to_dict() for x in self.pdf1_documents], "pdf2_documents": [x.to_dict() for x in self.pdf2_documents], "project_matches": [x.to_dict() for x in self.project_matches], "ahu_matches": [x.to_dict() for x in self.ahu_matches], "motor_comparisons": [x.to_dict() for x in self.motor_comparisons]}


def _discover_documents(paths: list[str | Path], side: str) -> list[BatchDocument]:
    documents=[]; seen_paths=set(); valid_paths=[]
    info("PDF master keşfi başladı", side=side, input_count=len(paths))
    for raw in paths:
...
def _files_for_ahu(documents, normalized_ahu):
    ...
    return tuple(document.path for document in documents if target in {normalize_equipment_id(x) for x in document.equipment})
...
def _extract_side_motors(paths, side, target_ahu):
    ...
    return records if target is None else [r for r in records if normalize_equipment_id(r.equipment_id)==target]
...
def _ahu_set(documents):
    return {normalize_equipment_id(e) for d in documents for e in d.equipment if normalize_equipment_id(e)}
...
def _best_project_for_document(document,left_groups):
    ...
    return best if best[0]>=0.50 else None
...
def analyze_batch(pdf1_paths,pdf2_paths):
    left_docs=_discover_documents(pdf1_paths,"PDF1"); right_docs=_discover_documents(pdf2_paths,"PDF2")
    left_groups=_group_documents(left_docs); right_groups=_group_documents(right_docs)
    ...
    for lk,docs in _infer_unresolved_right_documents(left_groups,right_docs,used_right_paths).items():
        ...
        lg=left_groups[lk]; p=lg[0].project; right_name=docs[0].project.project_name if docs[0].project.project_name else None
        overlap_total=sum(len(_ahu_set([d])&_ahu_set(lg)) for d in docs)
