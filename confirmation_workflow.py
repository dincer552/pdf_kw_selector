...
def _equipment_for_confirmation(document):
    """Use the master cache for real files, while retaining the small unit-test seam."""
...
def _project_confirmation_plan(left_groups, right_groups):
    ...
    for candidate, has_identity, identity_count, score, left_key, right_key, match, shared in sorted(candidates, key=lambda x: (x[1], x[2], x[3]), reverse=True):
        if not candidate or left_key in used_left or right_key in used_right or match.status == "EXACT":
            continue
        ...
        if _ask_project(left_name, right_name, shared):
            ...
            used_left.add(left_key); used_right.add(right_key)
    ...
    for project_name, left_group, right_group in project_pair_docs:
        ...
        for lid in list(unmatched_left):
            for rid in list(unmatched_right):
                lo = left_unique[lid]; ro = right_unique[rid]
                raw_left = lo.equipment_id; raw_right = ro.equipment_id
                if lid != rid: continue
                if raw_left == raw_right:
                    unmatched_left.discard(lid); unmatched_right.discard(rid); continue
                if flexible_rule is not None and _flexible_ahu_key(lid) == _flexible_ahu_key(rid):
                    flexible_auto.add((lid, rid)); unmatched_left.discard(lid); unmatched_right.discard(rid)
                    ...
                    approved.add((lid, rid)); flexible_rule = (_raw_ahu_key(raw_left), _raw_ahu_key(raw_right)); unmatched_left.discard(lid); unmatched_right.discard(rid)
                    ...
                    unmatched_left.discard(lid); unmatched_right.discard(rid)
