...
SUMMARY_FAN_MOTOR_POWER_RE = re.compile(r"fan\s+motor\s+power\s*(?:/\s*nominal\s+rpm\s*)?.*?(?P<supply_value>\d+(?:[.,]\d+)?)\s*\[?\s*kw\s*\]?\s*\(?\s*(?P<supply_quantity>\d+(?:[.,]\d+)?(?:\s*[x×]\s*\d+)?)\s*\)? .*?(?P<return_value>\d+(?:[.,]\d+)?)\s*\[?\s*kw\s*\]?\s*\(?\s*(?P<return_quantity>\d+(?:[.,]\d+)?(?:\s*[x×]\s*\d+)?)\s*\)?", re.IGNORECASE)
