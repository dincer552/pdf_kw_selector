...
def discover_motor_power_page(page_texts): return sorted([PageCandidate(i, score, _clean(t), matched) for i,t in enumerate(page_texts,1) for score,matched in [_page_score(t)]], key=lambda x:(-x.score,x.page_number))
def _normalize_quantity(q): return None if not q else re.sub(r"\s*[x×]\s*","x",q.strip())
def detect_component_type(text):
    lowered=_clean(text).lower(); supply=bool(re.search(r"\bsupply\s+air\b",lowered)); ret=bool(re.search(r"\breturn\s+air\b",lowered)); exh=bool(re.search(r"\bexhaust\s+air\b",lowered))
    if supply and not(ret or exh): return "Vantilatör","supply_fan"
    if exh and not supply: return "Aspiratör","exhaust_fan"
...
def _local_context(text,match):
    cleaned=_clean(text); before=cleaned[:match.start()]; directions=list(re.finditer(r"\b(?:supply|return|exhaust)\s+air\b",before,re.I)); start=directions[-1].start() if directions else max(0,match.start()-500); after=cleaned[match.end():]; nxt=re.search(r"\b(?:supply|return|exhaust)\s+air\b",after,re.I); end=match.end()+180 if not nxt else match.end()+nxt.start(); return cleaned[start:end]
def _result_from_match(text,page_number,match):
    cleaned=_clean(text); raw=match.group("value"); value=normalize_power(float(raw.replace(",",".")),"kw"); q=_normalize_quantity(match.group("quantity")); context=_local_context(cleaned,match); typ,role=detect_component_type(context); return MotorPowerResult(page_number,value,raw,q,"fan_motor_power","high" if role else "review",cleaned[max(0,match.start()-120):min(len(cleaned),match.end()+120)],typ,role,extract_equipment_id(cleaned))
...
def find_rated_motor_powers_in_pdf(path):
    ...
    for page_number,page in enumerate(PdfReader(str(path)).pages,1):
        for r in extract_rated_motor_powers_from_page(page.extract_text() or "",page_number):
            key=(r.page_number,r.component_role,r.value_kw,r.quantity)
            if key not in seen: seen.add(key); results.append(r)
    return results
